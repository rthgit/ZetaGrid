# 🦅 KAGGLE V4 (BULLETPROOF)

## 🔥 ANALISI DEL PROBLEMA
Non stai sbagliando nulla tu.
È Kaggle che "blocca" i file `.so` in memoria. Anche se li ricompili, lui usa la versione vecchia finché non riavvii il Kernel.

## ✅ SOLUZIONE
Ho modificato lo script per creare **un nome di file casuale** ogni volta (tipo `libzeta_8372.so`).
Così Kaggle è OBBLIGATO a caricare quello nuovo.

## 1. Copia-Incolla QUESTO (E sovrascrivi tutto):

```python
# ==========================================
# 🦅 ZETAGRID v10: KAGGLE REAL-DATA (V4)
# ==========================================
import os
import time
import ctypes
...
# HACK: Create random filename to avoid Kaggle caching old .so
lib_name = f"libzeta_v10_{random.randint(1000,9999)}.so"
cpp_name = "zeta_v10.cpp"

with open(cpp_name, "w") as f:
    f.write(cpp_source)

print(f"🔨 Compiling as {lib_name}...")
cmd = f"g++ -shared -fPIC -O3 {cpp_name} -o {lib_name} -lOpenCL"
subprocess.check_call(cmd, shell=True)
print("✅ Compiled.")

# Load the NEW random library
...
if lib.InitGPU(0) != 0: 
    print("❌ GPU Init Failed (Is Accelerator ON?).")
    exit()
...
if lib.AllocModelAndData(GB, data_ptr, len(tokens)) != 0:
    print("❌ Alloc Failed (Try reducing GB).")
    exit()
...
print(f"🧬 Training Started on {GB*4} Billion Parameters (WikiText-2)...")
while True:
    gen += 1
    seed = gen * 0.123
    ...
    if gen % 100 == 0:
        dt = time.time() - start
        hz = gen / dt
        print(f"Gen {gen} | Speed: {hz:.2f} Hz | Loss: {loss:.6f} | Params: {GB*4}B")
```
