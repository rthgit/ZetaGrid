...
## 🐛 MEMORIA PIENA
Hai chiesto 6GB + 6GB = 12GB per scheda.
La scheda ne ha 16GB, **MA** Kaggle riserva circa 4-5GB per il sistema operativo/CUDA.
Quindi: 16 - 5 = 11GB liberi. Noi ne chiedevamo 12.
**CRASH.**

## ✅ SOLUZIONE (V9)
Ho abbassato la richiesta a **5GB** (10GB totali usati per scheda).
Questo ci sta SICURO.
Totale Parametri: **20 MILIARDI (GPU 0) + 20 MILIARDI (GPU 1) = 40 MILIARDI.**
È comunque immenso.
...
```python
# ==========================================
# 🦅 ZETAGRID v10: KAGGLE DUAL-GPU (V9 SAFE)
# ==========================================
import os
import time
import ctypes
...
extern "C" {
    ...
    __attribute__((visibility("default"))) int AllocModelAndData(int size_gb, void* host_data_void, int data_len) {
        ...
        // MUST BE MULTIPLE OF 64 FOR SAFETY, round up
...
def train_island(gpu_id):
    ...
    # SAFE LIMIT: 5GB per GPU (10GB Total Usage per Card)
    GB = 5 
    BYTES = GB * 1024**3

    print(f"🧠 [GPU {gpu_id}] Allocating {GB*4} Billion Params (20B)...")
    if lib.AllocModelAndData(GB, data_ptr, len(tokens)) != 0:
        print(f"❌ [GPU {gpu_id}] Alloc Failed (Try 4GB).")
        return
