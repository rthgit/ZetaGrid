...
typedef struct {
    ...
    XKv_page_ops_uio_map maps[ MAX_UIO_MAPS ];
} XKv_page_ops_uio_info;

/***************** Variable Definitions **************************************/
static XKv_page_ops_uio_info uio_info;
...
static int uio_info_read_name(XKv_page_ops_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKv_page_ops_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKv_page_ops_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKv_page_ops_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKv_page_ops_Initialize(XKv_page_ops *InstancePtr, const char* InstanceName) {
	XKv_page_ops_uio_info *InfoPtr = &uio_info;
