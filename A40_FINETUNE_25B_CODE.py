#!/usr/bin/env python3
"""
ZETAGRID 25B - CODE FINE-TUNING
Specialized code model using 150K Python dataset
"""
...
print("=" * 70)
print("ZETAGRID 25B - CODE FINE-TUNING")
print("=" * 70)
...
OUTPUT_MODEL = f"{BASE_DIR}/zetagrid_25b_code_specialist.npy"

# Training config
BATCH_SIZE = 4096
SEQ_LEN = 256  # Longer for code
START_GEN = 0
TARGET_GEN = 50000  # 50K generations for code specialization
MUTATION_RATE = 0.003  # Lower for fine-tuning
...
print("\n" + "=" * 70)
print("PHASE 2: LOADING CODE DATASET")
print("=" * 70)

print(f"Loading: {CODE_DATASET}")
code_data_np = np.fromfile(CODE_DATASET, dtype=np.uint8)
print(f"✅ Loaded: {len(code_data_np)/1e9:.2f}GB ({len(code_data_np)/1e6:.0f}M tokens)")

print("Transferring to GPU...")
code_data = cp.array(code_data_np, dtype=np.uint8)
del code_data_np
...
while gen < TARGET_GEN:
    ...
    if gen % 5000 == 0:
        ...
        np.save(f"{BASE_DIR}/zetagrid_25b_code_checkpoint.npy", genome_cpu)
        del genome_cpu
...
print(f"\n💾 Saving FINAL code specialist model...")
genome_cpu = cp.asnumpy(genome_best)
